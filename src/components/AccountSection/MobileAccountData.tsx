...
export default function MobileAccountData({ BackState, BackSet }: { BackState: boolean, BackSet: any })
{
    ...
    return (
        <div>
            {GameHooks.GameState.game === "2048" && (
                <div className={ClassSwicher(GameHooks.GameState.parentDiv)}>
                    <GamePuzzle/>
                </div>
            )}

            {GameHooks.GameState.game === "TicTacToe" && (
                <div className={ClassSwicher(GameHooks.GameState.parentDiv)}>
                    <GameTicTac/>
                </div>
            )}
                    ...
                    <div className={ClassSwicher(GameHooks.GameState.parentDiv)}>
                        <div className="GameBarMobile" style={{ opacity: 1 }}>
                            <button className="m-10" onClick={() =>
                            {
                                ...
                                <button className="button-29" role="button"><Game2048 className="h-20 w-20 m-8"/>
                                </button>
                            </button>
                            <button onClick={() =>
                            {
                                ...
                                <button className="button-29" role="button"><TicTac className="h-20 w-20 m-8"/></button>
                            </button>
                        </div>
                    </div>
                </div>
            ...
            {GameHooks.GameState.parentDiv === "DontShow" && (
                <div>
                        ...
                        <div className='parent'>
                            ...
                            <div className="child">
                                <LocalizationProvider dateAdapter={AdapterDayjs}>
                                    <DateCalendar sx={{ width: "90%" }} className="maxAccount"
